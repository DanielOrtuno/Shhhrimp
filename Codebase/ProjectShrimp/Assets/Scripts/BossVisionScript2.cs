...
public class BossVisionScript2 : MonoBehaviour
{
    ...
    private void OnTriggerEnter(Collider other)
    {
        if (other.tag == "Player")
        {
            ...
            if (isSpotted)
            {
                ...
                playerXPosition = new Vector3(other.transform.position.x, bossWeapon.transform.position.y, 1);
            }
    ...
    void Update()
    {
        if (gameObject.transform.parent.GetComponent<FinalBossStage2>().bossHealth != 0)
        {
            if (isSpotted != true)
            {
